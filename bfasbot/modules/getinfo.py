...
from bfasbot import BOT, cmds, timedate
...
@BOT.on_message(Filters.command("chats", cmds) & Filters.me)
...
@BOT.on_message(Filters.command("unread", cmds) & Filters.me)
...
@BOT.on_message(Filters.command("id", cmds) & Filters.me)
def get_file(bot: BOT, message: Message):
    ...
    if message.reply_to_message:
        ...
        if rep.audio:
            ...
            forward_from = rep.forward_from_chat.id
        elif rep.forward_from:
            forward_from = rep.forward_from.id


    if not file_id:
      if forward_from is not None:
        message.edit("Forwarded chat ID:\n`{}`".format(forward_from))
      else: 
        repp = message.reply_to_message
        if repp:
          message.edit("This users's ID is:\n`{}`".format(repp.from_user.id))
        else:
          cmd = " ".join(message.command[1:])
          if cmd == "me":
            message.edit("My chat ID is:\n`{}`".format(message.from_user.id))
          else:
